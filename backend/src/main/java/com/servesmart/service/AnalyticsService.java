...
import com.servesmart.repository.*;
...
public class AnalyticsService {
    ...
    public byte[] exportToExcel(LocalDateTime start, LocalDateTime end) throws Exception {
        try (XSSFWorkbook workbook = new XSSFWorkbook()) {
            ...
            Long restaurantId = TenantContext.getCurrentTenantAsLong();
            List<com.servesmart.entity.RestaurantOrder> orders = (restaurantId != null ? orderRepository.findAllByRestaurantId(restaurantId) : orderRepository.findAll()).stream()
                .filter(o -> o.getCreatedAt().isAfter(start) && o.getCreatedAt().isBefore(end))
                .toList();
            int rowIdx = 1;
            for (com.servesmart.entity.RestaurantOrder order : orders) {
